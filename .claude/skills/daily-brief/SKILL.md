...
- `kind: json` (东方财富) → substitute `{since}`/`{today}` in `url`, fetch JSON, keep rows whose
  ...
  with the Read tool). Attribute the view to the broker (`orgSName`) — create
...
- Never write an institution view without a source page; never write a source page without
  the raw text saved.
