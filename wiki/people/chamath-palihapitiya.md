---
type: person
role: investor
affiliation: "Social Capital · All-In Podcast"
aliases: [Chamath Palihapitiya]
tags: [vc, host, all-in, ai]
created: 2026-06-21
updated: 2026-06-21
---

# Chamath Palihapitiya

## Who
Founder of Social Capital; co-host of the [[all-in-podcast|All-In Podcast]]. A recurring voice
in this wiki — track his evolving theses here.

## Views & style
Macro/structural; pro-capital-mobility, anti-wealth/property-tax. Pushes a "**KYC / audit
trail**" view of AI access.

## Notable views (this episode)
- **[[ai-gatekeeping]] thesis (durable hyperscaler oligopoly):** Frontier-Lab mistrust →
  governments let **hyperscalers** ([[amzn]]/MSFT/GOOG) gatekeep AI (KYC, compliance) → an AI
  **oligopoly**; neoclouds get squeezed. Key claim: the compliance moat is **unreplicable** so
  the lock-in **sticks**. (Agrees with [[david-friedberg]] that the pull toward concentration is
  real; they split only on whether it *lasts* — see [[ai-gatekeeping]].)
- On [[spcx]]: highest price/sales was at listing; it will "grow into" the valuation.
- On Iran MOU: "market's going to the moon" (risk-on).
- `[2026-06-27]` **AI as "the great leveler"** (a Travis-Kalanick-in-your-pocket for everyone);
  blames AI's bad "brand ambassadors" for the socialist surge. Running a **data-center project**
  (eyeing Tesla "Megapod"-type modular units); bullish **distributed inference** ([[orbital-compute]]);
  ex-Groq investor. — [[2026-06-27-allin-china-ai-micron-memory]]

## Sources
- [[2026-06-20-all-in-oligarchs-fable-iran]]
- [[2026-06-27-allin-china-ai-micron-memory]]
