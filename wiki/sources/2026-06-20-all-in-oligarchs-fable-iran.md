---
type: source
source_type: youtube
title: "World's First Trillionaire, Anthropic Fable Banned, The New Oligarchs, Iran Peace Deal"
author: "[[all-in-podcast]]"
publisher: "All-In Podcast"
url: https://www.youtube.com/watch?v=3Amlu4y94Ho
published: 2026-06-20
ingested: 2026-06-21
raw: "raw/World's First Trillionaire, Anthropic Fable Banned, The New Oligarchs, Iran Peace Deal.md"
tags: [news, macro, ai, regulation, ipo, space, geopolitics, oil]
---

# All-In — Trillionaire / Fable Ban / Oligarchs / Iran (2026-06-20)

A news/discussion episode (not a pitch). Investment-relevant threads extracted below;
political/philosophical commentary kept as attributed host opinion.

> ⚠️ **Sourcing caveat.** Much of the Anthropic segment is **contested, second-hand
> reporting** (cited to Washington Post / Semafor) and opinion. **[[david-sacks]] is the
> sitting White House AI czar** giving a "readout" of a government action — a direct conflict
> of interest. Treat claims as *attributed*, not established fact. **Hard figures were
> fact-checked 2026-06-21** — see the [Fact-check](#fact-check-2026-06-21) section (the SpaceX
> raise and Iran "$300B" are corrected there).

## TL;DR
[[spcx|SpaceX]] completed a record IPO (~$2T cap) and is acquiring Cursor; [[elon-musk]] is
now "the world's first trillionaire" (paper). The US government forced [[anthropic]] to pull
its **Fable 5** model over a national-security/jailbreak dispute — fueling a thesis
([[ai-gatekeeping]]) that **hyperscalers become AI gatekeepers**. An Iran peace **MOU**
(Strait of Hormuz reopening) is read as risk-on / bullish oil-flow.

## 1. New Oligarchs / "politburo" / learned helplessness `[02:43]`
Macro-political monologue ([[david-friedberg]], [[chamath-palihapitiya]]): warning against
state expansion, wealth taxes, and erosion of private property (cites Illinois "Pritzker's
law" taxing crypto/property). Framed as makers vs takers. **Opinion, not actionable** — kept
for host worldview context. Investment-adjacent nugget: hostility to wealth/property taxes.

## 2. SpaceX IPO + Cursor + trillionaire `[14:27]`
- **[[spcx|SpaceX]] IPO** (~June 11–12, 2026): priced **$135/sh**, filled the green shoe for an
  **~$85B raise** (⚠️ actually ~$75B — see Fact-check) (≈3× Saudi Aramco 2019); closed **+19%
  to $161**, **cap >$2T**; ~**$177** at
  taping. Briefly 4th-most-valuable (passed AMZN, MSFT), then ~7th (behind TSMC). 2025 revenue
  ~$19B. ~20–30% of the IPO allocated to **retail** (~600–700k Robinhood users).
- **Cursor acquisition:** [[spcx]] exercised an option to buy **Cursor** (~$4B revenue) for
  **$60B** (~15× sales; Chamath's math ≈ effectively ~$15B in stock terms). Backstory: Cursor
  ran on [[anthropic|Claude]], then Anthropic shipped a competing coding agent → Cursor built
  its own model on Elon's "Colossus" hardware.
- **[[elon-musk]]** = "world's first trillionaire" — [[david-sacks]] stresses it's **paper
  wealth** (1-yr lockup, not cash). Tesla/SpaceX **merger** floated; Elon swapped ~$300M Tesla
  options for shares.

## 3. Anthropic Fable 5 ban `[33:34]` — ⚠️ contested
- Commerce Sec **Howard Lutnik** ordered [[anthropic]] to restrict **Fable 5** to US citizens
  (days after a June 9 launch); unable/unwilling to geo-restrict that granularly, Anthropic
  **shut it down for everyone**.
- "**Mythos**" = the powerful model held 30 days for cyber testing; **Fable 5** = guardrailed
  public version. **Amazon** ([[andy-jassy]], an Anthropic investor + AWS host) flagged a
  **jailbreak**; [[dario-amodei]] called it narrow/not serious (and published a blog
  distinguishing jailbreak severity).
- Per **WaPo/Semafor** (as relayed by [[david-sacks]]): Anthropic expanded a "trusted partners"
  preview to ~50 firms without consulting the WH, including a group the WH deemed a China-linked
  national-security risk (**reported as SK Telecom**; Sacks declined to confirm). A Treasury-
  level call reportedly asked Dario to pull Fable; he declined until the export-control letter
  came.
- **Political subtext** (disputed): Reed Hoffman backing, Anthropic ↔ Trump-admin "beef," a
  Pete Hegseth ("Dept of War") tweet. Sacks denies political motivation; [[jason-calacanis]]
  pushes back that beef clearly exists but steelmans that identical steps from Gemini/OpenAI
  would've been approved.
- **Investment takeaway → [[ai-gatekeeping]]:** [[chamath-palihapitiya]] argues Frontier-Lab
  mistrust hands **hyperscalers** (AMZN/MSFT/GOOG) a gatekeeper/KYC moat 护城河 (cartel/oligopoly);
  [[david-friedberg]] counters that market forces drive **open-source + stack fragmentation**
  (IBM-mainframe→disaggregation analogy).

## 4. "Claude psychoanalyzes Dario" `[1:01:26]`
[[chamath-palihapitiya]] prompted an LLM (Claude) to analyze [[dario-amodei]]'s essays and the
Mythos events, yielding an "**epistemic exceptionalism**" critique. ⚠️ **Entertainment /
unverifiable opinion about a real person** — recorded as a segment, not as fact.

## 5. Iran war MOU + market impact `[1:14:47]`
- Iran war (began Feb 28, 2026; ~110 days) → **MOU**, formal signing **June 19, Geneva**,
  mediated by Pakistan. Terms: 60-day ceasefire extension (+ Lebanon), **Strait of Hormuz
  reopening**, Iran gives up enriched uranium under IAEA supervision + freezes program 60 days,
  sanctions lifted + **$300B reconstruction** (⚠️ reporting says ~$24–25B frozen assets — see
  Fact-check) (Gulf-funded, not US), US drawdown.
- **Market read:** Chamath — "market's going to the moon"; Hormuz reopening → **oil flows**
  (risk-on). Unresolved: Israel sign-on, ballistic missiles, future enrichment.

## Entities
Assets: [[spcx]], [[anthropic]], [[amzn]], [[tsla]] _(stub)_, [[cursor]] _(stub)_ ·
People: [[elon-musk]], [[chamath-palihapitiya]], [[david-sacks]], [[david-friedberg]],
[[dario-amodei]], [[jason-calacanis]] _(stub)_, [[andy-jassy]] _(stub)_ ·
Concepts: [[ai-gatekeeping]] · [[oil]] _(stub)_

## Fact-check (2026-06-21)
Verified the episode's hard claims (web sources). Detail on the entity pages.
- **[[spcx|SpaceX IPO]] — ✅ mostly:** $135/sh, +19% to $161, >$2T cap, passed Amazon — all
  confirmed. **⚠️ Correction:** raise ~**$75B**, not "$85B" (~$10B overstated).
- **Cursor deal — ✅:** $60B all-stock, ~$4B ARR (~15×), Q3 close — confirmed.
- **[[anthropic|Fable 5 ban]] — ✅ corroborated:** Commerce order, Amazon-flagged jailbreak
  (codebase→vuln tool), and **SK Telecom** named by multiple outlets (China ties = suspicion,
  not proven). The Treasury-refusal & political-beef framing remain Sacks's contested account.
- **Iran MOU — ✅ framework, ⚠️ two corrections:** MoU/Geneva/June 19, Hormuz reopening, 60-day
  ceasefire all confirmed. But the figure is **~$24–25B frozen Iranian assets** (possibly
  redirected to Gulf reconstruction) — **not the episode's "$300B reconstruction."** And
  **enriched-uranium removal is NOT yet agreed** — it's an unresolved item for the 60-day
  talks, despite the episode presenting it as done.
- **❓ Still unverified:** SpaceX 2025 revenue ~$19B / retail allocation; Pakistan as mediator;
  Illinois "Pritzker's law" specifics.
